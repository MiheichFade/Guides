...
1. В открывшемся окне заполнить следующие поля:  
    ...
    - Пропустить авматическую установку  
    ...
    Если на текущем этапе создания ВМ был указан ISO-образ, доступна автоматическая установка ОС.  
    [Подробнее об автоматической устаноке гостевой ОС](#автоматическая-установка).  

    В ином случае после создания ВМ будет необходимо [установить ОС самостоятельно](#ручная-установка).  

1. <a name="continue_create_vm">test</a>
...
Автоматическая установка ОС доступна только при указании ISO-образа на этапе создания ВМ. 
Также требуется проверить, что опция **Пропустить автоматическую установку** выключена.  
...
на этапе создания ВМ.  

1. На следующем этапе заполнить следующие поля:  
...
1. Нажать на кнопку 
<span style="border:2px solid #555; border-radius:0px; width: 200px; text-align:center; padding:3px;">Далее</span>
и [вернуться к созданию ВМ](#continue_create_vm).  
